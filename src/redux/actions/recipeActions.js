...
export const fetchRecipes = (query = '', category = '', dietary = []) => async (dispatch) => {
  try {
    const params = new URLSearchParams({
      q: query || 'recipe', // Default to 'recipe' if no query is provided
      app_id: 'a5de3521',
    ...
    if (category) params.append('category', category); // Include category filter if available
    if (dietary.length) params.append('diet', dietary.join(',')); // Join dietary filters
